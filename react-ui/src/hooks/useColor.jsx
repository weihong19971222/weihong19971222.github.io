...
export const useColor = () => {
  const them = {
    'primary': {'bgColor':'#1961B6','color':'#FFFFFF'},
    'secondary': {'bgColor':'#6c757d','color':'#FFFFFF'},
    'warning': {'bgColor':'#ffc107','color':'#000000'}
  }
  const disableColor = {'bgColor':'#E0E0E0','color':'#000000'}
  const makeColor = ({ themeColor, isDisabled }) => {
    if (isDisabled) return disableColor;
    if (them[themeColor] != undefined) return them[themeColor];
    const madeColor = (isColorDarkOrLight(themeColor) === 'light')?'#000000':'#FFFFFF';
    return {'bgColor':themeColor,'color':madeColor};
  };
